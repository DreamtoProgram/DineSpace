...
# 1. Create frontend/tray_return.html
tray_html = """<!DOCTYPE html>
<html lang="en" class="dark">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>DineSpace — Tray Return</title>
  ...
  <style>
    ...
    .laser-line {
      ...
      height: 3px;
      background: #FF5E1E;
      box-shadow: 0 0 14px 3px rgba(255, 94, 30, 0.95), 0 0 28px 7px rgba(255, 94, 30, 0.45);
    ...
    .laser-line-success {
      ...
      box-shadow: 0 0 18px 5px rgba(16, 185, 129, 0.95), 0 0 32px 8px rgba(16, 185, 129, 0.5) !important;
      animation: none !important;
      top: 50% !important;
    }

    /* Outer glowing card matching designed_pages/page_6.png */
    .scanner-outer-card {
      width: 640px;
      height: 550px;
...
<body class="bg-[#070B11] text-white min-h-screen flex overflow-x-hidden selection:bg-[#FF5E1E] selection:text-white">
  ...
  <main class="flex-1 flex flex-col min-h-screen overflow-y-auto">
    ...
    <!-- Top Header Bar (Height: 84px matching designed_pages/page_6.png) -->
    ...
    <div class="px-14 pt-8 pb-12 flex-1 flex flex-col justify-between max-w-[1280px]">
      
      <!-- Back Link & Titles -->
      <div class="space-y-3">
        <a href="my_visit.html" class="inline-flex items-center gap-2.5 text-sm font-medium text-gray-400 hover:text-white transition-colors group">
        ...
        <div class="pt-1">
          <h1 class="text-[34px] font-extrabold text-white tracking-tight leading-tight">Tray Return</h1>
          <p class="text-sm text-[#8B9BB4] mt-1 font-normal">Scan the QR code at the tray return counter to end your visit and free your seat.</p>
        </div>
      </div>

      <!-- Main Two-Column Row (Scanner Card on Left + 'How it works?' on Right) -->
      <div class="flex items-center gap-10 my-auto py-2">
        
        <!-- Left: Scanner Viewfinder Card (640px wide by 550px high) -->
        <div id="scanner-interactive-card" class="scanner-outer-card p-10 flex flex-col items-center justify-center relative cursor-pointer" title="Tap scanner to simulate tray return immediately">
          
          <!-- Inner Scanner Viewfinder Box (402px wide x 368px high) -->
          <div id="viewfinder-box" class="w-[402px] h-[368px] rounded-[24px] bg-[#121620] border border-white/5 relative flex items-center justify-center overflow-hidden shadow-2xl">
            
            <!-- 4 Bright Orange Curved SVG Corner Brackets with Rounded Caps -->
            <!-- Top-Left Bracket -->
            <svg class="absolute top-5 left-5 w-14 h-14 pointer-events-none" viewBox="0 0 56 56" fill="none">
              <path d="M4 52 V20 C4 11.1634 11.1634 4 20 4 H52" stroke="#FF5E1E" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>

            <!-- Top-Right Bracket -->
            <svg class="absolute top-5 right-5 w-14 h-14 pointer-events-none" viewBox="0 0 56 56" fill="none">
              <path d="M4 4 H36 C44.8366 4 52 11.1634 52 20 V52" stroke="#FF5E1E" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>

            <!-- Bottom-Left Bracket -->
            <svg class="absolute bottom-5 left-5 w-14 h-14 pointer-events-none" viewBox="0 0 56 56" fill="none">
              <path d="M4 4 V36 C4 44.8366 11.1634 52 20 52 H52" stroke="#FF5E1E" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>

            <!-- Bottom-Right Bracket -->
            <svg class="absolute bottom-5 right-5 w-14 h-14 pointer-events-none" viewBox="0 0 56 56" fill="none">
              <path d="M52 4 V36 C52 44.8366 44.8366 52 36 52 H4" stroke="#FF5E1E" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>

            <!-- Animated Glowing Orange Laser Line -->
            <div id="laser-line" class="laser-line"></div>

            <!-- Crisp Rounded QR Code (210px x 213px) -->
            <div class="w-[210px] h-[213px] bg-white rounded-[18px] p-2.5 shadow-2xl flex items-center justify-center relative z-0">
              <img src="assets/images/clean_qr_code.png" alt="Tray Return Counter QR" class="w-full h-full object-contain rounded-xl" />
            </div>

          </div>

          <!-- Instruction & Status Text matching page_6.png -->
          <div class="text-center mt-7 space-y-1">
            <p id="status-instruction" class="text-sm text-gray-200 font-medium tracking-wide">
              Position your QR code inside the frame
            </p>
            <p id="status-subtext" class="text-xs text-[#8B9BB4] font-normal">
              Scanning...
            </p>
          </div>
        ...
        <!-- Right: 'How it works?' Card (400px wide by 390px high) -->
        <div class="how-it-works-card p-8 flex flex-col justify-center space-y-6">
          <h2 class="text-[20px] font-bold text-white tracking-tight -mt-1">
            How it works?
          </h2>
          ...
          <div class="space-y-6">
            <!-- Step 1 -->
            <div class="flex items-center gap-4">
              <div class="w-11 h-11 rounded-full bg-[#2A1B14] flex items-center justify-center font-bold text-base text-white flex-shrink-0">
                1
              </div>
              <p class="text-sm text-gray-300 font-normal leading-relaxed">
                Scan the QR code at the tray return counter.
              </p>
            </div>
            ...
            <div class="flex items-center gap-4">
              ...
              <p class="text-sm text-gray-300 font-normal leading-relaxed">
                We’ll end your visit and free your seat.
              </p>
            </div>
            ...
            <div class="flex items-center gap-4">
              ...
              <p class="text-sm text-gray-300 font-normal leading-relaxed">
                You’ll see a confirmation once it’s done.
              </p>
            </div>
          </div>
        </div>
  ...
  <script src="js/tray_return.js"></script>
</body>
</html>
"""

with open("../frontend/tray_return.html", "w", encoding="utf-8") as f:
    f.write(tray_html.strip())

# 2. Create frontend/js/tray_return.js
tray_js = """/**
...
with open("../frontend/js/tray_return.js", "w", encoding="utf-8") as f:
    f.write(tray_js.strip())

print("Successfully generated tray_return.html and tray_return.js")
