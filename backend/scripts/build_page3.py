...
# 1. Create frontend/entry_scan.html
scan_html = """<!DOCTYPE html>
<html lang="en" class="dark">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>DineSpace — Entry Scan</title>
  ...
  <style>
    ...
    /* Outer glowing card matching designed_pages/page_3.png */
    .scanner-outer-card {
      width: 628px;
      height: 508px;
      ...
      box-shadow: 0 0 55px -6px rgba(255, 94, 30, 0.22), inset 0 0 24px -10px rgba(255, 94, 30, 0.05);
      border-radius: 32px;
      transition: all 0.3s ease;
    }

    .scanner-outer-card:hover {
      box-shadow: 0 0 68px -4px rgba(255, 94, 30, 0.28), inset 0 0 28px -8px rgba(255, 94, 30, 0.08);
    }
  </style>
</head>
<body class="bg-[#070B11] text-white min-h-screen flex overflow-x-hidden selection:bg-[#FF5E1E] selection:text-white">
  ...
  <main class="flex-1 flex flex-col min-h-screen overflow-y-auto">
    ...
    <!-- Top Header Bar (Height: 84px matching designed_pages/page_3.png) -->
    ...
    <div class="px-14 pt-8 pb-12 flex-1 flex flex-col justify-between">
      ...
      <div class="space-y-3">
        ...
        <div class="pt-1">
          <h1 class="text-[32px] font-extrabold text-white tracking-tight leading-tight">Entry Scan</h1>
          <p class="text-sm text-[#8B9BB4] mt-1 font-normal">Scan your university ID / QR code at the entrance to get your seat.</p>
        </div>
      </div>

      <!-- Main Center QR Viewfinder Card (Matching designed_pages/page_3.png exactly: 628px by 508px) -->
      <div class="flex justify-center items-center my-auto py-2">
        <div id="scanner-interactive-card" class="scanner-outer-card p-10 flex flex-col items-center justify-center relative cursor-pointer" title="Tap scanner to simulate check-in immediately">
          ...
          <div id="viewfinder-box" class="w-[402px] h-[368px] rounded-[24px] bg-[#121620] border border-white/5 relative flex items-center justify-center overflow-hidden shadow-2xl">
            ...
            <div class="w-[210px] h-[213px] bg-white rounded-[18px] p-2.5 shadow-2xl flex items-center justify-center relative z-0">
              <img src="assets/images/clean_qr_code.png" alt="University Dining QR Pass" class="w-full h-full object-contain rounded-xl" />
            </div>

          </div>

          <!-- Instruction Text Exactly as in designed_pages/page_3.png -->
          <p id="status-instruction" class="text-sm text-[#8B9BB4] font-medium tracking-wide text-center mt-7">
      ...
      <!-- Spacer to match vertical balance -->
      <div class="h-4"></div>
  ...
  <script src="js/entry_scan.js"></script>
</body>
</html>
"""

with open("../frontend/entry_scan.html", "w", encoding="utf-8") as f:
    f.write(scan_html.strip())

# 2. Create frontend/js/entry_scan.js
scan_js = """/**
  ...
  async function performEntryScan() {
    ...
    try {
      ...
      if (err.status === 409) {
        ...
        if (statusInstruction) {
          statusInstruction.textContent = err.message || 'Scan verification failed. Please tap again.';
          statusInstruction.className = 'text-sm text-red-400 font-semibold tracking-wide text-center mt-7';
        }
        isScanning = false;
      }
    }
...
with open("../frontend/js/entry_scan.js", "w", encoding="utf-8") as f:
    f.write(scan_js.strip())

print("Successfully updated entry_scan.html and entry_scan.js")
